...
const EditMember = ({ onClose, teamId, member, onMemberUpdated }) => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            const response = await fetch(
                `http://localhost:5001/api/teams/${teamId}/members/${member.user._id}`,
                {
                    method: "PATCH",
                    headers: {
                    ...
                    body: JSON.stringify({ role, title, status }),
                },
            );
            ...
            <div onClick={(e) => e.stopPropagation()} className="flex bg-white p-6 rounded-md h-fit w-fit">
                <form onSubmit={handleSubmit} className="flex flex-col w-[400px] gap-3">
                    <div className="flex flex-row gap-2">
                        <Pencil />
                        <p className="flex flex-row gap-2 font-bold">Edit Member</p>
                        <button
                            type="button"
